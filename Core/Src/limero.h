...
class SinkFunction: public Sink<T> {
	...
	public:
		SinkFunction(std::function<void(const T &t)> func) {
			_func = func;
		}
		void on(const T &t) {
			_func(t);
...
class ArrayQueue {
	private:
		...
		struct alignas(cache_line_size) AlignedAtomicU64: public std::atomic<
		INDEX_TYPE> {
...
class TimerSource: public Source<TimerMsg>, public Named {
	...
	public:
		TimerSource(Thread &thr, uint32_t interval = UINT32_MAX, bool repeat =
				false, const char *name = "unknownTimer1")
				:
				Named(name) {
			_interval = interval;
			_repeat = repeat;
		...
		void on(const T &t) {
			if (_thread) {
				if (_queue.push(t))
					...
					WARN("push failed on '%s' [%d/%d] ", name(), _queue.size(), _queue
							.capacity());
					stats.bufferOverflow++;
				}
			} else {
...
class Cache: public Flow<T, T>, public Sink<TimerMsg> {
	...
	public:
		Cache(Thread &thread, uint32_t minimum, uint32_t maximum, bool request =
				false)
				:
				_thread(thread), _minimum(minimum), _maximum(maximum), _timerSource(thread) {
...
class LambdaFlow: public Flow<IN, OUT> {
	...
	public:
		...
		;
		LambdaFlow(std::function<bool(OUT&, const IN&)> func)
				:
				_func(func) {
		}
		;
		void lambda(std::function<bool(OUT&, const IN&)> func) {
			_func = func;
		}
...
class Filter: public Flow<T, T> {
	...
	public:
		...
		;
		Filter(std::function<bool(const T&)> func)
				:
				_func(func) {
		}
		;
			...
			_pollInterval
					>> [&](const TimerMsg) {
						if (_requestables.size() && connected())
							_requestables[_idx++ % _requestables.size()]
									->request();
					};
