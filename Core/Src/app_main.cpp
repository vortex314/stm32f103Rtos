...
public:
	Uart(Thread &thread, UART_HandleTypeDef *huart) : Actor(thread), _huart(huart)
	{
	...
	bool init()
	{
		...
		HAL_UART_Receive_DMA(huart, _raw, sizeof(_raw));
	}
	bool sendFrame(Bytes &bs)
	{
		Bytes buffer = frame(tx.bytes());
		if (HAL_UART_Transmit_DMA(&huart2, buffer.data(), buffer.size()) != HAL_OK)
			_txdOverflow++;
	}
	void onFrame(RxdFunction func, void *instance);
	void UART_Data_Process(UART_HandleTypeDef *huart)
	{
		...
		if (_wrPtr != _rdPtr)
		{
			if (_wrPtr > _rdPtr)
			{
				for (int i = _rdPtr; i < _wrPtr; i++)
					add(_raw[i]);
			}
			else
			{
				for (int i = _rdPtr; i < sizeof(_raw); i++)
					add(_raw[i]);
				if (_wrPtr > 0)
					for (int i = 0; i < _wrPtr; i++)
						add(_raw[i]);
			}
			rdPtr = wrPtr;
		}
	...
	Source<Bytes> &incoming() { return _framesRxd; };
	Sink<Bytes> &outgoing(){};
	uint32_t errors() { return _txdOverflow; }
} uart2(&huart2);

void sendNode(const char *topic)
{
	if (tx.reset().array().add(B_NODE).add(topic).close().addCrc().ok())
		uart2.sendFrame(tx.bytes());
}
