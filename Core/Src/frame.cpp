...
#define PPP_MAX_FRAME_SIZE 2000
*/
// FCS lookup table
...
bool ppp_deframe(Bytes &out, const Bytes &in) {
  ...
  bool fcsCheck = fcs.result() == 0x0F47;
  /*  if (!fcsCheck)
      LOGW(" FCS incorrect %x vs %x in [%d]", fcs.result(), 0x0F47,
     in.size());*/
  out.pop_back();
  out.pop_back();
  return !escFlag && fcsCheck;
}
